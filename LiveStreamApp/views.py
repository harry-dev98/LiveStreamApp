...
def _utils_is_valid_session(peer, sessId):
    ...
    start_time = sess.startTime - dt.timedelta(minutes=15)
    end_time = start_time + dt.timedelta(minutes=sess.interval+15)
...
def _utils_check_session_time(request, peer, sessId):
    ...
    sess = peer.sess
    dDir = "{}/{}/".format(sess, str(dt.date.today()))
    ...
    if sess.isActive == True and peer.who == "Teacher":
        return render(request, "LiveStreamApp/error.html", {'error':"Session is being used by some other host"})
    time_now = dt.datetime.now()
    start_time = sess.startTime - dt.timedelta(minutes=15)
    end_time = start_time + dt.timedelta(minutes=sess.interval+15)
    # print(time_now.replace(tzinfo=None),start_time.replace(tzinfo=None), end_time.replace(tzinfo=None))
    if time_now.replace(tzinfo=None) > end_time.replace(tzinfo=None):
        ...
        if peer.who == "Teacher":
            return render(request, "LiveStreamApp/host.html", {"sess":peer.sess.sessId, "id":peer.id, "name" : peer.name, "dDir" : dDir})
        else:
            return render(request, "LiveStreamApp/peer.html", {"sess":peer.sess.sessId, "id":peer.id, "name":peer.name, "time":sess.startTime, "interval":sess.interval, "dDir" : dDir})
...
def host(request, name, id):
    sess_dir = os.path.join(MR, name)
    if not os.path.isdir(sess_dir):
        os.mkdir(sess_dir)
    data_dir = os.path.join(sess_dir, str(dt.date.today()))
    if not os.path.isdir(data_dir):
        os.mkdir(data_dir)
        os.mkdir(os.path.join(data_dir, "videos"))
        os.mkdir(os.path.join(data_dir, "documents"))
    try:
